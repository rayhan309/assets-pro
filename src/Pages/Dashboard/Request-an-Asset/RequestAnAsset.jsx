...
const RequestAnAsset = () => {
  ...
  const { data: assets = [], isLoading } = useQuery({
    queryKey: ["assets", user?.email, limit, currentPage],
    queryFn: async () => {
      const resp = await axiosSquer.get(
        `/all-assets?limit=${limit}&skip=${parseInt(currentPage) * limit}`
      );
      const totaleAsset = resp.data.assetsCount;
      const page = Math.ceil((totaleAsset * limit) / 100);
      setTotalePage(page);
      // console.log(page);
      ...
      <div className="px-10 pb-10 pt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {availableAssets.map((asset) => (
            <div
              key={asset._id}
              className="rounded-xl my-bg bg-secondary/10 shadow-sm p-4 flex flex-col"
            >
              <img
                src={asset.productImage}
                alt={asset.productName}
                className="max-h-[190px] w-full object-cover rounded-lg mb-4"
              />

              <span className="text-lg font-semibold text-secondary">
                {asset.productName}
              </span>
              <span className="text-sm my-2 text-secondary">
                Type: <span className="">{asset.productType}</span>
              </span>
              <span className="text-sm mb-2 text-secondary">
                Available: <span className="">{asset.availableQuantity}</span>
              </span>
              <span className="text-sm mb-2 text-secondary">
                Company: <span className="">{asset.hrCompanyName}</span>
              </span>

              <button
                onClick={() => setSelectedAsset(asset)}
                className="my-btn-glossy"
              >
                Request
              </button>
            </div>
        ...
        <div className="flex justify-center items-center gap-2 mt-16">
          ...
          {[...Array(totalePage)].map((a, i) => {
            return (
              <button
                onClick={() => {
                  setCurrentPage(i);
                }}
                className={`btn ${currentPage === i ? "bg-purple-400" : "bg-purple-600"}`}
              >
                {i + 1}
              </button>
            );
          ...
          {
            // totalePage !== currentPage - 3 && 
            <button
              className={`btn bg-purple-400 ${totalePage === currentPage + 1 && "hidden"}`}
              onClick={() => {
                setCurrentPage(currentPage + 1);
              }}
            >
