...
import { FaEnvelope, FaTrashAlt, FaBriefcase } from "react-icons/fa";
    ...
    <div className="p-4 sm:p-6 lg:p-10 min-h-screen">
      ...
      >
        ...
        <p className="text-gray-400 mt-2 flex items-center justify-center md:justify-start gap-2">
          <FaBriefcase /> Total Active Members: <span className="text-white font-bold">{uniqueEmployees.length}</span>
        </p>
      </motion.div>
...
const EmployeeCard = ({ employee, refetch, allAssets }) => {
  const { employeeImage, employeeName, employeeEmail, assignmentDate, _id, companyName } = employee;
  const axiosSecure = useAxiosSquer();
  ...
  const handleRemoveEmployee = (employeeEmail, companyName ) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.delete(`/approvedAssets?email=${employeeEmail}&companyName=${companyName}`).then(() => {
          refetch();
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="w-full mt-6 grid grid-cols-1 gap-3">
             <div className="bg-slate-900/50 p-3 rounded-lg border border-white/5 text-center">
                <p className="text-[10px] uppercase tracking-widest text-gray-500 font-bold">Last Assignment</p>
                <p className="text-sm text-gray-300 font-medium">{formattedDate}</p>
             </div>
